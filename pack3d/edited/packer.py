...
class Packer:
    ...
    def pivot_dict(self, bin, item):
        """For each item to be placed into a certain bin, obtain a corresponding comparison parameter 
        of each optional pivot that the item can be placed.
        Args:
    ...
    def pack(
        ...
        # for unplaced_item in self.unplaced_items:   # 最开始所有盒子都在这里
        ...
        for bin in self.bins: 
            for unplaced_item in self.unplaced_items: 
                bin.unplaced_items.append(unplaced_item) # 把待放置的item信息传入bin
        
        for bin in self.bins:  # 对每个bin进行装箱
            for unplaced_item in self.unplaced_items:
